...
namespace Gurux.DLMS
{
    class GXDLMSChipperingStream
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine(BitConverter.ToString(SBoxInverse));
        ...
        /// <returns></returns>
        static UInt32 ToUInt32(byte[] data, int offset)
        {
            return (uint)(data[offset] | data[++offset] << 8 | data[++offset] << 16 | data[++offset] << 24);            
        }

        static UInt32 SubWord(uint x)
        {
            return (uint)SBox[x & 0xFF] | (((uint)SBox[(x >> 8) & 0xFF]) << 8) |
                (((uint)SBox[(x >> 16) & 0xFF]) << 16) | (((uint)SBox[(x >> 24) & 0xFF]) << 24);
        }
        ...
        /// <returns></returns>
        UInt32 Shift(UInt32 r, int shift)
        {
        ...
        private static readonly uint[] CryptoTable1 =
		{
        ...
        private static readonly uint[] CryptoTable2 =
		{
        ...
        private static readonly uint[] CryptoTable3 =
		{
        ...
        private static readonly uint[] CryptoTable4 =
		{
        ...
        internal static void GetUInt32(uint n, byte[] data, int offset)
        {
            data[offset] = (byte)(n);
            data[++offset] = (byte)(n >> 8);
            data[++offset] = (byte)(n >> 16);
        ...
        /// <param name="n"></param>
        internal static void ShiftRight(uint[] block, int n)
        {            
            uint bit = 0;
            for (int i = 0; i < 4; ++i)
            {
        ...
        /// <param name="bs"></param>
        /// <returns></returns>
        internal static uint[] GetUint128(byte[] bs)
        {
            uint[] us = new uint[4];
            us[0] = BEToUInt32(bs, 0);
            us[1] = BEToUInt32(bs, 4);
            us[2] = BEToUInt32(bs, 8);
            us[3] = BEToUInt32(bs, 12);
            return us;
        }
        ...
        void MultiplyH(byte[] x)
        {
            ...
            UInt32_To_BE(z[0], x, 0);
            UInt32_To_BE(z[1], x, 4);
            UInt32_To_BE(z[2], x, 8);
            UInt32_To_BE(z[3], x, 12);
        }        
        ...
        byte[] gCTRBlock(byte[] buf, int bufCount)
        {
            for (int i = 15; i >= 12; --i)
            {
                if (++counter[i] != 0)
        ...
        static void packLength(ulong len, byte[] bs, int off)
        {
            UInt32_To_BE((uint)(len >> 32), bs, off);
            UInt32_To_BE((uint)len, bs, off + 4);
        }
        ...
        /// <returns></returns>
        static bool TagsEquals(byte[] tag1, byte[] tag2)
        {
        ...
        public virtual void Write(byte[] input, int offset, int count)
        {
            ...
            {
                bufBlock[bufOff++] = input[offset + i];
        ...
        {            
            ...
            {
                byte[] tmp = new byte[BlockSize];
                Array.Copy(bufBlock, 0, tmp, 0, bufOff);
                output = gCTRBlock(tmp, bufOff);
            }

            // Final gHASH
            byte[] X = new byte[16];
            packLength((ulong)Aad.Length * 8UL, X, 0);
            ...
            if (!Encrypt)
            {
                if (Security != Security.Encryption && !TagsEquals(this.Tag, tag))
                {
